        ...
        for index, row in prem_df.iterrows():
            ...
            if not is_motor:
                ...
                if mot_df.empty:
                    ...
                    if not mot_row.empty:
                        prev_ins = str(mot_row['PREVIOUS_INSURER_NAME'].iloc[0]).strip().upper()
            ...
            if not is_motor:
                ...
                if not mot_df.empty:
                    mot_row = mot_df[mot_df['POLICY_NUMBER'] == pol_num]
                    if not mot_row.empty:
                        m_prod = str(mot_row['PRODUCT_NAME'].iloc[0]).upper()
                        m_class = str(mot_row['CLASS_OF_VEHICLE'].iloc[0]).upper()
                        m_body = str(mot_row['BODY_TYPE'].iloc[0]).upper()
                        m_gvw = pd.to_numeric(mot_row['GROSS_VEHICLE_WEIGHT'].iloc[0], errors='coerce')
